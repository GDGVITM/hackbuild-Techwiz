...
export function AuthProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.authenticated && data.user) {
          ...
          console.log('AuthContext - Token from cookies:', cookieToken ? 'exists' : 'not found');

          if (cookieToken) {
            setToken(cookieToken);
          } else {
            console.log('AuthContext - No token found in cookies, authentication may be incomplete');
          }
        } else {
          console.log('AuthContext - User not authenticated from server');
          // Clear any stale data
      ...
      // Check if it's a timeout error
      if (error.name === 'AbortError') {
        console.log('AuthContext - Authentication check timed out, checking cookies');
        // Try to get auth data from cookies as fallback
        const cookieToken = Cookies.get('authToken');

        if (cookieToken) {
          console.log('AuthContext - Using token from cookies');
          setToken(cookieToken);
          // Note: We don't have user data in cookies, so we set authenticated to true
          // but the user data will be missing until a successful server check
          setIsAuthenticated(true);
        } else {
          // Clear any stale data
          setUser(null);
  ...
  const login = (token: string, user: AuthUser) => {
    ...
    console.log('AuthContext - Login completed, token stored in cookies');
  };
